...
Descripci�n: Actualiza los registros en la tabla de PREORDENES.tblPreOrdenes.
...
--WITH ENCRYPTION
@pstrInfosesion			VARCHAR(2000) = Null
AS 
...
BEGIN TRY
	...
	SELECT @intInconsistencias = 1, @strOpcion = 'PreOrdenes', @strProceso= 'Actualizar', @strProcedimiento = OBJECT_NAME(@@PROCID), @strObjeto = '', @pstrUsuario=ISNULL(@pstrUsuario,'') 	-- Asignar valor a los par�metros para identIFicar la opci�n ejecutada
	...
	BEGIN	
		-- Instrucci�n para consultar el mensaje que se debe retornar
		...
		BEGIN
			...
			SET @strXLM='<PreOrden>'
						...
						+'<Accion>Ingreso/Modificaci�n de PreOrden</Accion>'
			...
			-- Notificar que la actualizaci�n finaliz� exitosamente
		...
		BEGIN
			-- Notificar que la actualizaci�n NO finaliz� exitosamente
	...
	ELSE
	BEGIN
		-- Notificar que la actualizaci�n NO se realiz�
