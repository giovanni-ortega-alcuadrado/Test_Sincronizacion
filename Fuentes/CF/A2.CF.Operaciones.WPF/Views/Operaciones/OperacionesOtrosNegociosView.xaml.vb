...
Partial Public Class OperacionesOtrosNegociosView
    Inherits UserControl
    Public objVMOperaciones As OperacionesOtrosNegociosViewModel
    ...
    Private Sub IniciarFormulario(ByVal plogInicializarModal As Boolean)
        Try
            ...
            Me.DataContext = New OperacionesOtrosNegociosViewModel
            If plogInicializarModal Then
                objVMOperaciones = CType(Me.DataContext, OperacionesOtrosNegociosViewModel)
            End If
            InitializeComponent()
            ...
            'Me.ScrollForma.Height = Application.Current.MainWindow.ActualHeight - 235
    ...
    Private Async Sub OtrosNegocios_Loaded(ByVal sender As Object, ByVal e As System.Windows.RoutedEventArgs) Handles Me.Loaded
        Try
            If mlogInicializado = False Then
                ...
                objVMOperaciones.logEsModal = logEsModal
                objVMOperaciones.HabilitarDuplicar = True
                objVMOperaciones.ViewOperacionesOtrosNegocios = Me
    ...
    Private Sub BuscadorClienteListaButon_finalizoBusqueda(pstrClaseControl As String, pobjComitente As OYDUtilidades.BuscadorClientes)
        Try
            ...
            Try
                If Not IsNothing(objVMOperaciones) Then
                    If IsError(pobjComitente.IdComitente) Then
                        hasError = True
                    End If
                End If
            Catch
            ...
            If hasError = False Then
                If Not IsNothing(objVMOperaciones) Then
                    Me.objVMOperaciones.BusquedaOperacionesOtrosNegocios.Cliente = pobjComitente.IdComitente
                End If
            End If
    ...
    Private Sub btnRefrescarPantalla_Click(sender As Object, e As RoutedEventArgs)
        Me.objVMOperaciones.RecargarPantalla()
    End Sub

    Private Sub Buscador_Especies_nemotecnicoAsignado(pstrNemotecnico As String, pstrNombreEspecie As String)
        Try
            'Para el timer de ordenes
            If Not IsNothing(objVMOperaciones) Then
                Me.objVMOperaciones.BusquedaOperacionesOtrosNegocios.Especie = pstrNemotecnico
            End If
        Catch ex As Exception
            mostrarErrorAplicacion(Program.Usuario, "Se presentó un error al asignar la especie.", Me.Name, "Buscador_Especies_nemotecnicoAsignado", Program.TituloSistema, Program.Maquina, ex, Program.RutaServicioLog)
        End Try
    End Sub
