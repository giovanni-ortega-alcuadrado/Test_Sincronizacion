...
Partial Public Class CruceOperacionesManualView
...
#Region "Variables"
    ...
    Private mobjVM As CruceOperacionesManualViewModel
...
#Region "Inicializacion"
    Public Sub New()
        ...
        Me.DataContext = New CruceOperacionesManualViewModel
InitializeComponent()
    End Sub
    ...
    Private Async Sub inicializar()
        If Not Me.DataContext Is Nothing Then
            mobjVM = CType(Me.DataContext, CruceOperacionesManualViewModel)
    ...
    Private Sub ucBtnDialogoImportar_CargarArchivo(sender As A2ComunesControl.ObjetoInformacionArchivo, strProceso As String)
        Try
            ...
            If Not IsNothing(objDialog.pFile) Then
                ...
                Dim viewImportacion As New cwCargaArchivos(CType(Me.DataContext, CruceOperacionesManualViewModel), strRutaArchivo, strProceso, mobjVM.STRARCH_TITULOS_MVTOS, False)
...
#Region "Eventos para asignar cliente y especie"
    ...
    Private Sub chkCruzarPortafolioTodos_Click(sender As Object, e As RoutedEventArgs)
        Try
            Dim chk As CheckBox = CType(sender, CheckBox)
            Dim check As Boolean = chk.IsChecked.Value

            mobjVM.SeleccionarTodoPortafolio(check)

            If Not IsNothing(mobjVM.DetalleSeleccionado) Then
                If Not mobjVM.DetalleSeleccionado.logCruzarLiquidacionVenta Then
                    mobjVM.DetalleSeleccionado.logCruzarLiquidacionVenta = True
                End If
            End If
        Catch ex As Exception
    ...
    Private Sub chkCruzarPortafolio_Click(sender As Object, e As RoutedEventArgs)
        Try
            If Not IsNothing(mobjVM.DetalleSeleccionado) Then
                If Not mobjVM.DetalleSeleccionado.logCruzarLiquidacionVenta Then
                    mobjVM.DetalleSeleccionado.logCruzarLiquidacionVenta = True
    ...
    Private Sub chkCruzarLiquidacionVenta_Click(sender As Object, e As RoutedEventArgs)
        Try
            If Not IsNothing(mobjVM.DetalleSeleccionado) Then
                If Not mobjVM.DetalleSeleccionado.logCruzarLiquidacionVenta Then
                    For Each li In mobjVM.ListaEncabezadoPortafolio
                        li.logCruzarPortafolio = False
                    Next
                End If
            End If
        Catch ex As Exception
    ...
    Private Sub btnLimpiarFiltroPortafolio_Click(sender As Object, e As RoutedEventArgs)
        Try
            Me.txtFiltroPortafolio.Text = String.Empty
        Catch ex As Exception
            A2Utilidades.Mensajes.mostrarErrorAplicacion(Program.Usuario, "Se presentó un error al asignar el elemento seleccionado", Me.Name, "btnLimpiarFiltroPortafolio_Click", Program.TituloSistema, Program.Maquina, ex, Program.RutaServicioLog)
        End Try
    End Sub

    Private Async Sub btnFiltroPortafolio_Click(sender As Object, e As RoutedEventArgs)
        Try
            Await mobjVM.FiltrarPortafolio(Me.txtFiltroPortafolio.Text)
        Catch ex As Exception
            A2Utilidades.Mensajes.mostrarErrorAplicacion(Program.Usuario, "Se presentó un error al aplicar el filtro del ISIN", Me.Name, "btnFiltroVenta_Click", Program.TituloSistema, Program.Maquina, ex, Program.RutaServicioLog)
        End Try
    End Sub

    Private Async Sub txtFiltroPortafolio_KeyDown(sender As Object, e As KeyEventArgs)
        Try
            If e.Key = Key.Tab Then
                Await mobjVM.FiltrarPortafolio(Me.txtFiltroPortafolio.Text)
            End If
    ...
    Private Async Sub txtFiltroPortafolio_KeyUp(sender As Object, e As KeyEventArgs)
        Try
            If e.Key = Key.Tab Then
                Await mobjVM.FiltrarPortafolio(Me.txtFiltroPortafolio.Text)
            End If
...
#Region "Manejadores error"
    ...
    Private Sub btnConsultarLiquidaciones_Click(sender As Object, e As RoutedEventArgs)
        mobjVM.ConsultarLiquidaciones()
    End Sub
...
Private Sub btnExportarExcel_Click(sender As Object, e As RoutedEventArgs)
        mobjVM.ExportarPortafolioExcel()
    End Sub
