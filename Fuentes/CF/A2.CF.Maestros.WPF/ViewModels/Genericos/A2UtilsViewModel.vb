...
#End If
        Try
            ...
            pstrNombreClaseCombos = pstrClaseCombos
...
#Region "Métodos acceso datos sincronicos"
    ...
    'se adiciona el parametro logRecargaRecursos para determinar si debe recargar el diccionario
    Private Async Function consultarCombos(ByVal pstrClaseCombos As String, ByVal pstrNombreControl As String, ByVal pstrUsuario As String, Optional ByVal logRecargaRecursos As Boolean = False) As Task(Of Boolean)
            ...
            If Not objCombos Is Nothing Then
                If objCombos.Count > 0 Then
                    ...
                    Else
                        ...
                        End If
                        Application.Current.Resources.Add(pstrNombreControl, dicListaCombos.ToDictionary(Function(k) k.Key, Function(k) k.Value.ToList()))
                        ...
                        Dim objListaNombresCombos As New List(Of String)
                        If Application.Current.Resources.Contains(Program.NombreListaCombosEspecificos) Then
                            objListaNombresCombos = CType(Application.Current.Resources(Program.NombreListaCombosEspecificos), List(Of String))
                            Application.Current.Resources.Remove(Program.NombreListaCombosEspecificos)
                        End If
                        objListaNombresCombos.Add(pstrNombreControl)
                        Application.Current.Resources.Add(Program.NombreListaCombosEspecificos, objListaNombresCombos)
                    End If
                End If
            End If
    ...
    End Function
    ''' <summary>
    ''' Método que realiza la carga de combos
    ''' </summary>
    ''' <history>
    ''' Modificado por   : Jhonatan Acevedo (Alcuadrado S.A.)
    ''' Descripción      : Modificado para que retorne el usuario, necesario para otorgar permisos sobre formatos.
    ''' Fecha            : Marzo 30/2015
    ...
    ''' Descripción      : Modificado el nivel de protección a público para uso directo de combos con clase específica
    ''' Fecha            : Febrero 02/2016
    ''' Pruebas CB       : Javier Eduardo Pardo Moreno (Alcuadrado S.A.) - Febrero 02/2016 - Resultado Ok 
    ''' </history>
    Public Async Function CargarCombosAsync(ByVal pstrClaseCombos As String, ByVal pstrUsuario As String) As Task(Of ObservableCollection(Of RIAWeb.OYDUtilidades.ItemCombo))
        ...
        Try
            ...
            If pstrClaseCombos.Equals(String.Empty) Then
                objTask = Await dcProxy.Load(dcProxy.cargarCombos_UsuarioQuery(pstrUsuario:=Program.Usuario, pstrInfoConexion:=Program.HashConexion)).AsTask()
            Else
