...
''' Pantalla Detalle Monedas País (Maestros)
''' </summary>
''' <remarks>Natalia Andrea Otalvaro (Alcuadrado S.A.) - 29 de Enero 2018</remarks>
''' <history>
'''
'''</history>
Public Class Pais_MonedaViewModel
...
#Region "Variables - REQUERIDO"
    ...
    '---------------------------------------------------------------------------------------------------------------------------------------------------
    Private mobjEncabezadoPorDefecto As tblPais_Moneda
    ...
    '---------------------------------------------------------------------------------------------------------------------------------------------------
    Private mobjEncabezadoAnterior As tblPais_Moneda
    Private mobjEncabezadoGuardadoAnterior As tblPais_Moneda
    Public viewListaPrincipal As Pais_MonedaView
...
#Region "Propiedades del Encabezado - REQUERIDO"
    ...
    ''' </summary>
    Private _ListaEncabezado As List(Of CPX_tblPais_Moneda)
    Public Property ListaEncabezado() As List(Of CPX_tblPais_Moneda)
        Get
    ...
    ''' </summary>
    Private _EncabezadoSeleccionado As CPX_tblPais_Moneda
    Public Property EncabezadoSeleccionado() As CPX_tblPais_Moneda
        Get
            Return _EncabezadoSeleccionado
        End Get
        Set(ByVal value As CPX_tblPais_Moneda)
    ...
    ''' </summary>
    Private WithEvents _EncabezadoEdicionSeleccionado As tblPais_Moneda
    Public Property EncabezadoEdicionSeleccionado() As tblPais_Moneda
        Get
            Return _EncabezadoEdicionSeleccionado
        End Get
        Set(ByVal value As tblPais_Moneda)
            _EncabezadoEdicionSeleccionado = value
            If IsNothing(_EncabezadoEdicionSeleccionado) Then
                DescripcionMoneda = String.Empty
            Else
                If String.IsNullOrEmpty(_EncabezadoEdicionSeleccionado.strCodigoMoneda) Then
                    DescripcionMoneda = String.Empty
                Else
                    If _EncabezadoSeleccionado.intID > 0 Then
                        If Not IsNothing(_EncabezadoSeleccionado) Then
                            DescripcionMoneda = _EncabezadoSeleccionado.strNombreMoneda
                        Else
                            DescripcionMoneda = String.Empty
                        End If
                    Else
    ...
    Public Property DescripcionMoneda() As String
        ...
        Set(ByVal value As String)
            ...
            MyBase.CambioItem("DescripcionMoneda")
        End Set
    End Property
...
#Region "Métodos públicos del encabezado - REQUERIDO"
    ...
    Public Sub CrearNuevoDetalle(Optional ByVal plogCopiarValorRegistroAnterior As Boolean = False)
        Try
            Dim objNvoEncabezado As New tblPais_Moneda
            ...
            objNvoEncabezado.intIDPais = _IDRegistroEncabezado
    ...
    Public Async Function ActualizarRegistroDetalle() As Task(Of Boolean)
        ...
        Try
            ...
            Dim objRespuesta As InvokeResult(Of List(Of CPX_tblValidacionesGenerales)) = Await mdcProxy.Pais_Moneda_ValidarAsync(_EncabezadoEdicionSeleccionado.intID,
                                                                                                                          _EncabezadoEdicionSeleccionado.strCodigoMoneda,
            ...
            If Not IsNothing(objRespuesta) Then
                If Not IsNothing(objRespuesta.Value) Then
                    ...
                    If (objListaResultado.Where(Function(i) i.logInconsitencia = True).Count) = 0 Then
                        ...
                        Await ConsultarEncabezado("BUSQUEDA", IDRegistroEncabezado, intIDRegistroActualizado)
                        plogRetorno = True
                    Else
        ...
        Try
            ...
            If CType(sender, A2Utilidades.wppMensajePregunta).DialogResult Then
                If Not IsNothing(_EncabezadoSeleccionado) Then
                    ...
                    Dim objRespuesta As InvokeResult(Of List(Of CPX_tblValidacionesGenerales)) = Await mdcProxy.Pais_Moneda_EliminarIDAsync(_EncabezadoSeleccionado.intID, Program.Usuario, Program.HashConexion)
    ...
    Public Sub AbrirDetalle()
        Try
            If logVentanaDetalleActiva = False Then
                Dim objViewDetalle As New Pais_MonedaFormView(Me)
...
#Region "Métodos privados del encabezado - REQUERIDOS"
    ...
    ''' <history>
    ''' </history>
    Private Sub ObtenerRegistroAnterior(ByVal pobjRegistro As tblPais_Moneda, ByRef pobjRegistroSalvar As tblPais_Moneda)
        Dim objEncabezado As tblPais_Moneda = New tblPais_Moneda

        Try
            If Not IsNothing(pobjRegistro) Then
                objEncabezado.intID = pobjRegistro.intID
                objEncabezado.intIDPais = pobjRegistro.intIDPais
                objEncabezado.strCodigoMoneda = pobjRegistro.strCodigoMoneda
...
#Region "Métodos sincrónicos del encabezado - REQUERIDO"
    ...
    Private Async Sub consultarEncabezadoPorDefecto()
        Try
            Dim objRespuesta As InvokeResult(Of tblPais_Moneda)

            objRespuesta = Await mdcProxy.Pais_Moneda_ConsultarDefectoAsync(Program.Usuario, Program.HashConexion)
    ...
    Private Async Function ConsultarEncabezado(ByVal pstrOpcion As String,
                                               ByVal pintIDRegistroPadre As Integer,
        ...
        Try
            IsBusy = True
            Dim objRespuesta As InvokeResult(Of List(Of CPX_tblPais_Moneda)) = Nothing

            ErrorForma = String.Empty

            objRespuesta = Await mdcProxy.Pais_Moneda_ConsultarAsync(pintIDRegistroPadre, Program.Usuario, Program.HashConexion)
    ...
    Private Async Function ConsultarEncabezadoEdicion(ByVal pintID As Integer) As Task(Of Boolean)
        ...
        Try
            IsBusy = True
            Dim objRespuesta As InvokeResult(Of tblPais_Moneda) = Nothing

            ErrorForma = String.Empty

            objRespuesta = Await mdcProxy.Pais_Moneda_ConsultarIDAsync(pintID, Program.Usuario, Program.HashConexion)
