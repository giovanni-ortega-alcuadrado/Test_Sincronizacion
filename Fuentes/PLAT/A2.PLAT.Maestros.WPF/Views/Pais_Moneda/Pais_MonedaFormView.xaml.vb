...
Partial Public Class Pais_MonedaFormView
    Inherits Window
...
#Region "Inicializacion"
    ''' <history>
    ''' </history>
    Public Sub New(ByVal pobjViewModel As Pais_MonedaViewModel)
        Try
            InitializeComponent()
...
#Region "Manejadores error"
    ...
    Private Sub ctlBuscadorGenericoMoneda_finalizoBusqueda(pstrClaseControl As String, pobjItem As OYDUtilidades.BuscadorGenerico)
        Try
            If Not IsNothing(pobjItem) Then
                mobjVM.EncabezadoEdicionSeleccionado.strCodigoMoneda = pobjItem.CodItem
                mobjVM.DescripcionMoneda = pobjItem.Nombre
            End If
        Catch ex As Exception
            A2Utilidades.Mensajes.mostrarErrorAplicacion(Program.Usuario, mobjVM.DiccionarioMensajesPantalla("GENERICO_ERRORREGISTRO"), Me.Name, "dgGrid_BindingValidationError", Program.TituloSistema, Program.Maquina, ex, Program.RutaServicioLog)
        End Try
    ...
    Private Sub btnLimpiarMoneda_Click(sender As Object, e As RoutedEventArgs)
        Try
            If Not IsNothing(mobjVM.EncabezadoEdicionSeleccionado) Then
                mobjVM.EncabezadoEdicionSeleccionado.strCodigoMoneda = String.Empty
                mobjVM.DescripcionMoneda = String.Empty
            End If
        Catch ex As Exception
            A2Utilidades.Mensajes.mostrarErrorAplicacion(Program.Usuario, mobjVM.DiccionarioMensajesPantalla("GENERICO_ERRORREGISTRO"), Me.Name, "dgGrid_BindingValidationError", Program.TituloSistema, Program.Maquina, ex, Program.RutaServicioLog)
        End Try
