...
Public Class OrdenPago_DetalleChequesViewModel
...
#Region "Declaraciones"
    ...
    Private strParametroTipoCruceGMF As String = String.Empty
    ...
    Public Sub New()
        ...
        Catch ex As Exception
            A2Utilidades.Mensajes.mostrarErrorAplicacion(Program.Usuario, "Se presentó un problema en la creación de los objetos",
                                 Me.ToString(), "OrdenPago_DetalleChequesViewModel.New", Application.Current.ToString(), Program.Maquina, ex)
        End Try
    End Sub
...
#Region "Propiedades"
        ...
        Set(ByVal value As String)
            ...
            If logCambiarPropiedades Then
                If _TipoCliente = Program.Tipo_Cliente_Cliente Then
                    NombreBeneficiario = NombreComitente
                    TipoIdentificacion = TipoDocumentoComitente
                    NroDocumento = NroDocumentoComitente
                Else
    ...
    Public Property NombreBeneficiario() As String
        ...
        Set(ByVal value As String)
            ...
            MyBase.CambioItem("NombreBeneficiario")
        End Set
    End Property

    Private _HabilitarCampoNroDocumento As Boolean
    ...
    Public Property TipoCheque() As String
        ...
        Set(ByVal value As String)
            _TipoCheque = value
            If logCambiarPropiedades Then
                If _TipoCheque = GSTR_CHEQUE_GERENCIA Then
                    TipoCruce = Nothing
                End If
            ...
            End If
            HabilitarControles()
            MyBase.CambioItem("TipoCheque")
        End Set
    End Property

    Private _DescripcionTipoCheque As String
    Public Property DescripcionTipoCheque() As String
        Get
            Return _DescripcionTipoCheque
        End Get
        Set(ByVal value As String)
            _DescripcionTipoCheque = value
            MyBase.CambioItem("DescripcionTipoCheque")
        End Set
    End Property
...
#Region "Metodos"
            ...
            If Not IsNothing(objRespuesta) Then
                If Not IsNothing(objRespuesta.Value) Then
                    If objRespuesta.Value.Count > 0 Then
                        ...
                        NombreBeneficiario = objRespuesta.Value.First.strNombre
                        TipoIdentificacion = objRespuesta.Value.First.strTipoDocumento
                        NroDocumento = objRespuesta.Value.First.strNroDocumento
                        TipoCheque = objRespuesta.Value.First.strTipoCheque
                        TipoCruce = objRespuesta.Value.First.strTipoCruce
    ...
    Public Sub VerificarCobro_GMF(Optional ByVal plogSoloHabilitarTipo As Boolean = False)
        Try
            ...
            'CHEQUES
            If _TipoCheque = GSTR_CHEQUE Then
                'VERIFICA SÍ ES TERCERO O CLIENTE
                If TipoCliente = GSTR_TERCERO Then
                    ...
                    If logDebeCobrarGMF = False Then
                        ...
                        If Not String.IsNullOrEmpty(TipoCruce) Then
                            If TipoProducto = GSTR_FONDOS_TIPOPRODUCTO Then
                                logDebeCobrarGMF = True
                            Else
                                If a.Where(Function(i) i.Contains(TipoCruce)).Count > 0 Then
                                    For Each li In a.Where(Function(i) i.Contains(TipoCruce))
                                        If li = TipoCruce Then
                                            logDebeCobrarGMF = True
                                        End If
                                    Next
                                End If
                ...
                Else
                    ...
                    End If
                End If
            ElseIf _TipoCheque = GSTR_CHEQUE_GERENCIA Then
                If TipoProducto = GSTR_FONDOS_TIPOPRODUCTO And logEsFondosOYD And Not String.IsNullOrEmpty(strTipoGMF_TesoreriaFondosOYD) Then
                    HabilitarTipoGMF = False
                    If plogSoloHabilitarTipo = False Then
                        TipoGMF = strTipoGMF_TesoreriaFondosOYD
            ...
            Else
                If Not IsNothing(DiccionarioCombosOYDPlus) Then
                    If DiccionarioCombosOYDPlus.ContainsKey("CONCEPTOS") Then
                        If TipoCheque = GSTR_CHEQUE Then
    ...
    Private Sub HabilitarControles()
        Try
            If ConsultarValoresBaseDatos Then
                ...
                HabilitarDetalleConcepto = False
            ...
            Else
                ...
                If TipoCliente = Program.Tipo_Cliente_Cliente Then
                    HabilitarCampoBeneficiario = False
                    HabilitarCampoNroDocumento = False
                Else
                    HabilitarCampoBeneficiario = True
                    HabilitarCampoNroDocumento = True
                End If
    ...
    Public Function ValidarRegistro() As Boolean
        Try
            ...
            If IsNothing(TipoCliente) Then
                ...
                logTieneError = True
            End If
            If IsNothing(NombreBeneficiario) Or String.IsNullOrEmpty(NombreBeneficiario) Then
                strMensajeValidacion = String.Format("{0}{1} - Nombre Beneficiario.", strMensajeValidacion, vbCrLf)
                logTieneError = True
            Else
                Dim objValidacionExpresion = clsExpresiones.ValidarCaracteresEnCadena(NombreBeneficiario, clsExpresiones.TipoExpresion.Caracteres2)
            ...
            Else
                ...
                End If
            End If
            If (IsNothing(IDConcepto) Or IDConcepto <= 0) And (IsNothing(DetalleConcepto) Or String.IsNullOrEmpty(DetalleConcepto)) Then
                ...
                logTieneError = True
            End If
            If Not String.IsNullOrEmpty(DetalleConcepto) Then
                Dim objValidacionExpresion = clsExpresiones.ValidarCaracteresEnCadena(DetalleConcepto, clsExpresiones.TipoExpresion.Caracteres)
                If Not IsNothing(objValidacionExpresion) Then
                    If objValidacionExpresion.TextoValido = False Then
    ...
    Public Async Function GuardarRegistro() As Task(Of Boolean)
        Try
            ...
            If Not IsNothing(DiccionarioCombosOYDPlus) Then
                ...
                DescripcionTipoCheque = ObtenerDescripcionEnDiccionario("TIPOCHEQUE", _TipoCheque)

                If _TipoCheque = GSTR_CHEQUE_GERENCIA Then
                    DescripcionTipoCruce = String.Empty
                Else
                    DescripcionTipoCruce = ObtenerDescripcionEnDiccionario("TIPOCRUCE", _TipoCruce)
                End If
            ...
            If ValidarRegistro() Then
                If ConsultarValoresBaseDatos Then
                    ...
                    objRespuesta = Await dcProxy.OrdenPago_GuardarDetalle_TesoreroAsync(_IDEncabezado, _IDDetalle, "CHEQUE", _TipoCheque, _TipoCruce, strDetalleConcepto, strTipoGMF, IIf(strTipoGMF = "D", _ValorNeto, _ValorGenerar), _ValorGMF, IIf(strTipoGMF = "D", _ValorGenerar, _ValorNeto), Program.Usuario, Program.UsuarioWindows, Program.HashConexion, _IDConcepto)
                    If Not IsNothing(objRespuesta) Then
                        If Not IsNothing(objRespuesta.Value) Then
                            If objRespuesta.Value.Count > 0 Then
        ...
        Try
            If lo.HasError = False Then
                If lo.Entities.Count > 0 Then
                    ...
                    If DiccionarioCombosOYDPlusCompleto.ContainsKey("PARAMETROTIPOCRUCE_GMF") Then 'Parametro Tipos Cruce que requieren GMF - JDOL 20150804
                        If DiccionarioCombosOYDPlusCompleto("PARAMETROTIPOCRUCE_GMF").Count = 1 Then
                            strParametroTipoCruceGMF = DiccionarioCombosOYDPlusCompleto("PARAMETROTIPOCRUCE_GMF").FirstOrDefault.Retorno
                        End If
                    End If
