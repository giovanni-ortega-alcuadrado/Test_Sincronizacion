...
'Archivo: Public Class GenerarAutomaticamenteRCViewModel.vb
...
Public Class GenerarAutomaticamenteRCViewModel
    ...
    Public Sub New()
        ...
        Try
            If Not Program.IsDesignMode() Then
                ...
                End If
                objProxy.Load(objProxy.cargarCombosEspecificosQuery("Tesoreria_RecibosCaja", Program.Usuario, Program.HashConexion), AddressOf TerminoTraerConsecutivos, Nothing)
...
#Region "Metodos"
        ...
        Try
            ...
            If validaFechaCierre("Actualizar") Then
                If Validaciones() Then
                    IsBusy = False
                    dcProxy.Load(dcProxy.ConsultarUltimosValoresQuery("ING", "RC", _UltimosValoresSelected.NombreConsecutivos, _UltimosValoresSelected.CuentaContable, _UltimosValoresSelected.FechaCorte, _UltimosValoresSelected.Recibi, _UltimosValoresSelected.TipoId,
    ...
    Private Sub GuardarRC(Optional GraboEncabezado As Boolean = False, Optional GraboDetalle As Boolean = False)
        Try
            ...
            If ListaTesoreriaGA.ElementAt(intNroGuadar).Seleccionado Then
                ...
                If Not GraboEncabezado And intSecuencia = 1 Then
                    ...
                    IsBusy = True
                    dcProxy.InsertarRC(_UltimosValoresSelected.NombreConsecutivo, _UltimosValoresSelected.TipoIdentificacion, NroDocumento,
                ...
                If Not GraboDetalle Then
                    ...
                    IsBusy = True
                    dcProxy.InsertarRCDetalle(_UltimosValoresSelected.NombreConsecutivo, intNroCE, intSecuencia, Comitente, _ListaTesoreriaGA.ElementAt(intNroGuadar).TotalLiquidacion,
                                              _UltimosValoresSelected.IDCuentaContable, _ListaTesoreriaGA.ElementAt(intNroGuadar).Detalle,
                                              _ListaTesoreriaGA.ElementAt(intNroGuadar).NroDocumento, _UltimosValoresSelected.CCosto, Program.Usuario, Program.HashConexion,
            ...
            If intNroGuadar < (ListaTesoreriaGA.Count - 1) Then
                ...
                If intSecuencia > EspecificacionesCESelected.NroDetalles Then 'SLB20130509 Manejo para saber cuando se tiene que generar un nuevo encabezado, dependiendo del Nro de detalles por encabezado (Digitado en pantalla)
                    If intSecuencia > 1 Then 'Se ingresa el Cheque solo si hubo encabezado y al menos un registro
            ...
            Else
                If intSecuencia > 1 Then 'Se ingresa el Cheque solo si hubo encabezado y al menos un registro
                    InsertarCheque("GuardarChequeFinal")
                    Exit Sub
                End If
                IsBusy = False
                GeneroExitosamente()
    ...
    Public Sub ConsultarPendientesTesoreraia()
        Try
            ...
            dcProxy.Load(dcProxy.ConsultarPendientesTesoreriaQuery(_ParametrosConsultaSelected.Cumplimiento, _ParametrosConsultaSelected.FormadePago,
                                                                   ...
                                                                   _ParametrosConsultaSelected.Tipo, "RC", Program.Usuario, Program.HashConexion), AddressOf TerminoConsultarPendientesTesoreria, "Consulta")
    ...
    Public Sub ConsultarUltimosValores()
        Try
            ...
            dcProxy.Load(dcProxy.ConsultarUltimosValoresQuery("CON", "RC", Nothing, Nothing, Nothing, Nothing, Nothing,
    ...
    Public Function Validaciones() As Boolean
        ...
        If IsNothing(_UltimosValoresSelected.Consignacion) Then
            A2Utilidades.Mensajes.mostrarMensaje("Debe seleccionar una fecha para la consignación.", Program.TituloSistema, A2Utilidades.wppMensajes.TiposMensaje.Advertencia)
            Validaciones = False
            Return Validaciones
        End If
        ...
        If Not IsNothing(_ListaTesoreriaGA) Then
            If ListaTesoreriaGA.Count > 0 Then
                For Each objLista In ListaTesoreriaGA.Where(Function(i) i.Seleccionado = True)
                    If objLista.Seleccionado = True Then
    ...
    Private Sub InsertarCheque(ByVal InsertadoDesde As String)
        Try
            IsBusy = True
            dcProxy.InsertarCheque(_UltimosValoresSelected.NombreConsecutivo, intNroCE, 1, 0, _UltimosValoresSelected.BancoGirador, _UltimosValoresSelected.NumCheque,
                                   ValorTotalPorRC, _UltimosValoresSelected.IdBanco, _UltimosValoresSelected.Consignacion, Program.Usuario,
        ...
        Catch ex As Exception
            IsBusy = False
            A2Utilidades.Mensajes.mostrarErrorAplicacion(Program.Usuario, "Se presentó un problema al insertar el cheque del Recibo de Caja",
    ...
    Private Sub GeneroExitosamente()
        A2Utilidades.Mensajes.mostrarMensaje("Recibos de Caja insertados: " & CStr(intRegistrosInsetados) & vbCr & "Valor de: " & FormatCurrency(CStr(ValorTotalRegistrosInsertados), 2, , , TriState.True), Program.TituloSistema, A2Utilidades.wppMensajes.TiposMensaje.Exito)
...
#Region "MetodosAsincronicos"
    ...
    Private Sub TerminoTraerUltimosValores(ByVal lo As LoadOperation(Of OyDTesoreria.UltimosValores))
        ...
        If Not lo.HasError Then
            If lo.UserState.ToString = "Grabar" Then
                ...
                mostrarMensajePregunta("Desea generar los Recibos de Caja",
    ...
    Private Sub TerminoConsultarPendientesTesoreria(ByVal lo As LoadOperation(Of OyDTesoreria.TesoreriaGA))
        ...
        If Not lo.HasError Then
            ...
            Else
                A2Utilidades.Mensajes.mostrarMensaje("No hay operaciones para crear Recibos de Caja con los parámetros especificados.", Program.TituloSistema, A2Utilidades.wppMensajes.TiposMensaje.Advertencia)
            End If
        Else
                            ...
                            If Not IsNothing(_UltimosValoresSelected.NombreConsecutivo) Then
                                ...
                                If lo.Entities.First.InfoAdicional02.Equals("1") Then
                                    If lo.Entities.First.InfoAdicional03.Equals(strCompaniaConsecutivo) Then
                                        ...
                                        strCompaniaBanco = lo.Entities.First.InfoAdicional03
        ...
        Try
            If Not lo.HasError Then
                If objProxy.ItemCombos.Count > 0 Then
                    listConsecutivos = objProxy.ItemCombos.Where(Function(y) y.Categoria = "ConsecutivosTesoreriaRC").ToList
                End If
            Else
                IsBusy = False
...
#Region "Propiedades"
    ...
    Private _ParametrosConsultaSelected As New ParametrosConsultaRC
    Public Property ParametrosConsultaSelected As ParametrosConsultaRC
        Get
            Return _ParametrosConsultaSelected
        End Get
        Set(ByVal value As ParametrosConsultaRC)
...
Public Class ParametrosConsultaRC
...
Public Class EspecificacionesRC
    Implements INotifyPropertyChanged
