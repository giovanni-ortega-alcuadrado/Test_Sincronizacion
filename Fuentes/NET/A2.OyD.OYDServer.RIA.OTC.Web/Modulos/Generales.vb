...
Module Generales
    ...
    Dim db As New OyD_ImportacionesDataContext
    ...
    '    Public Function CampoTabla(ByVal plngIdCondicion As Object, ByVal pstrCampo As String, ByVal pstrTabla As String, ByVal pstrCondicion As String) As Object
    '        '/******************************************************************************************
    '        '/* INICIO DOCUMENTO
    '        '/* Function CampoTabla(ByVal plngIdCondicion As Variant, ByVal pstrCampo As String, ByVal pstrTabla As String, ByVal pstrCondicion As String) As Variant
    '        '/* Alcance     :   Public
    '        '/* Descripción :   Función utilizada cuando se desea consultar un solo campo de una tabla con tres condiciones
    '        '/*                 donde las dos primeras son fijas y la tercera es un string
    '        '/* Parámetros  :
    '        '/* Por             Nombre              Tipo            Descripción
    '        '/* Valor           plngIdCondicion     Variant         valor para la condición del WHERE
    '        '/* Valor           pstrCampo           string          campo de la tabla a retornar
    '        '/* Valor           pstrTabla           string          tabla a consultar
    '        '/* Valor           pstrCondicion       string          Nombre del campo de comparación en el WHERE

    '        '/* Valores de retorno:
    '        '/* CampoTabla : Devuelve el valor del campo consultado si encuentra datos
    '        '/*              Devuelve un caracter en blanco si NO encuentra datos.
    '        '/* FIN DOCUMENTO
    '        '/******************************************************************************************
    '        CampoTabla(plngIdCondicion, pstrCampo, pstrTabla, pstrCondicion)
    '        'On Error GoTo Err_CampoTabla
    '        ''Dim rst As rdoResultset
    '        'Dim MyType

    '        'MyType = TypeName(plngIdCondicion)

    '        'Dim gstrsql = "SELECT " & pstrCampo
    '        'gstrsql = gstrsql & " FROM " & pstrTabla
    '        'gstrsql = gstrsql & " WHERE lngIdComisionista = " & _
    '        'gstrsql = gstrsql & " AND lngIdSucComisionista = "
    '        'If MyType = "String" Then
    '        '    gstrsql = gstrsql & " AND " & pstrCondicion & " = '" & plngIdCondicion & "'"
    '        'Else
    '        '    gstrsql = gstrsql & " AND " & pstrCondicion & " = " & plngIdCondicion
    '        'End If

    '        'Dim ret = db.ExecuteQuery(Of Object)(gstrsql, Nothing)

    '        'rdoErrors.Clear()
    '        'rst = gdbCursorLocal.OpenResultset(gstrsql, rdOpenStatic, rdConcurReadOnly)


    '        'If ret.Count > 0 Then
    '        '    CampoTabla = ""
    '        'Else
    '        '    Select Case ret(0). .rdoColumns(0).Type
    '        '        Case rdTypeBIT
    '        '            CampoTabla = CInt(.rdoColumns(0).Value)
    '        '        Case Else
    '        '            CampoTabla = .rdoColumns(0).Value
    '        '    End Select
    '        'End If
    ...
    'Err_CampoTabla:
    '        'LogErrores("CampoTabla", Err.Number, Err.Description)
    '        Resume Exit_CampoTabla
    '    End Function
    ...
    Public Function ValorParametro(ByVal pstrParametro As String, ByVal pstrDefault As String) As String
        'Jorge Arango 2008/10/22

        '/******************************************************************************************
        '/* INICIO DOCUMENTO
        '/* Alcance     :   Public
        '/* Descripción :   Trae el valor de la tabla tblparametros y si arroja algun nulo, envia
        ...
        ValorParametro = pstrDefault
        Dim gstrsql = "SELECT strValor FROM tblParametros WHERE strParametro = '" & pstrParametro & "'"
        Dim ret = db.ExecuteQuery(Of String)(gstrsql)
        Dim resultado = ret(0)
        If Not IsNothing(resultado) Then
            'If ret.Count > 0 Then
            ValorParametro = resultado
            'End If
        End If

        'rst = gdbCursorLocal.OpenResultset(gstrsql, rdOpenKeyset, rdConcurReadOnly)
        'With rst
        '    If Not .BOF And Not .EOF Then
        '        ValorParametro = .rdoColumns(0).Value
        '    End If
        '    .Cancel()
        '    .Close()
        'End With
