...
''' Pantalla Configuración de recepctores (Maestros)
''' </summary>
''' <remarks>Santiago Upegui G. (Alcuadrado S.A.) - 15 de Julio 2015</remarks>

Public Class ConfiguracionReceptoresViewModel
...
#Region "Variables - REQUERIDO"
    ...
    '---------------------------------------------------------------------------------------------------------------------------------------------------
    Private mobjEncabezadoPorDefecto As ConfiguracionReceptores
    ...
    '---------------------------------------------------------------------------------------------------------------------------------------------------
    Private mobjEncabezadoAnterior As ConfiguracionReceptores
    Private pintIDRegistro As Integer = 0
#End Region
...
#Region "Inicialización - REQUERIDO"
    ...
    ''' Fecha            : Julio 16/2015
    ''' Pruebas CB       : Santiago Upegui G. (Alcuadrado S.A.) - Julio 16/2015 - Resultado Ok 
    ''' </history>
    Public Async Function inicializar() As Task(Of Boolean)
        ...
        Try
            ...
            If Not Program.IsDesignMode() Then
                ...
                Dim objItemsDispobibles As New List(Of Items)

                objItemsDispobibles.Add(New Items With {.Tipo = "R", .Descripcion = "Receptor"})
                objItemsDispobibles.Add(New Items With {.Tipo = "L", .Descripcion = "Representante Legal"})
                objItemsDispobibles.Add(New Items With {.Tipo = "A", .Descripcion = "Receptor y Representante Legal"})

                Tabladisponibles = objItemsDispobibles
...
#Region "Propiedades del Encabezado - REQUERIDO"
    ...
    ''' </summary>
    Private _ListaEncabezado As EntitySet(Of ConfiguracionReceptores)
    Public Property ListaEncabezado() As EntitySet(Of ConfiguracionReceptores)
        Get
            Return _ListaEncabezado
        End Get
        Set(ByVal value As EntitySet(Of ConfiguracionReceptores))
    ...
    ''' </summary>
    Private WithEvents _EncabezadoSeleccionado As ConfiguracionReceptores
    Public Property EncabezadoSeleccionado() As ConfiguracionReceptores
        Get
            Return _EncabezadoSeleccionado
        End Get
        Set(ByVal value As ConfiguracionReceptores)
    ...
    ''' </summary>
    Private _cb As CamposBusquedaConfiguracionReceptores
    Public Property cb() As CamposBusquedaConfiguracionReceptores
        Get
            Return _cb
        End Get
        Set(ByVal value As CamposBusquedaConfiguracionReceptores)
            _cb = value
    ...
    Public Overrides Async Sub TerminoSubmitChanges(ByVal So As SubmitOperation)
        ...
        Try
            ...
            Else
                ...
                ' Consultar los registros del encabezado incluyendo el Await para que el llamado sea sincrónico
                pintIDRegistro = EncabezadoSeleccionado.IdReceptores
                Await ConsultarEncabezado(True, String.Empty, "", Nothing, Nothing, Nothing, So.UserState.ToString)
            End If
        Catch ex As Exception
...
#Region "Métodos sincrónicos del encabezado - REQUERIDO"
    ...
    '            objRet = Await dcProxy.Load(dcProxy.ConsultarCalificacionesCalificadoraPorDefectoSyncQuery(Program.Usuario)).AsTask
    ...
    Private Async Function ConsultarEncabezado(ByVal plogFiltrar As Boolean,
                                               ...
                                               Optional ByVal pintSucursal As Integer = 0,
                                               Optional ByVal pintCodigoMesa As Integer = 0,
                                               Optional ByVal pstrNombre As String = Nothing,
                                               Optional ByVal pstrOpcion As String = "") As Task(Of Boolean)

        Dim logResultado As Boolean = False
        Dim objRet As LoadOperation(Of ConfiguracionReceptores)
        ...
        Try
            ...
            mdcProxy.ConfiguracionReceptores.Clear()
            ...
            If plogFiltrar Then
                ...
                objRet = Await mdcProxy.Load(mdcProxy.ConfiguracionReceptoresFiltrarSyncQuery(pstrfiltro:=pstrFiltro, pstrUsuario:=Program.Usuario, pstrInfoConexion:=Program.HashConexion)).AsTask()
            Else
                objRet = Await mdcProxy.Load(mdcProxy.ConfiguracionReceptoresConsultarSyncQuery(pCodigo:=pstrCodigo,
                                                                                                pidsucural:=pintSucursal,
                                                                                                pidmesa:=pintCodigoMesa,
                                                                                                Nombre:=pstrNombre,
            ...
            If Not objRet Is Nothing Then
                If objRet.HasError Then
                    ...
                    objRet.MarkErrorAsHandled()
                Else
                    ListaEncabezado = mdcProxy.ConfiguracionReceptores
                    If pstrOpcion = ValoresUserState.Ingresar.ToString Then
                        EncabezadoSeleccionado = ListaEncabezado.LastOrDefault
                    ElseIf pstrOpcion = ValoresUserState.Actualizar.ToString Then
                        EncabezadoSeleccionado = ListaEncabezado.Where(Function(i) i.IdReceptores = pintIDRegistro).FirstOrDefault
                    End If
...
#Region "Métodos públicos del encabezado - REQUERIDO"
    ...
    Public Overrides Sub NuevoRegistro()
        ...
        Dim objNvoEncabezado As New ConfiguracionReceptores
        ...
        Try
            ...
            If Not mobjEncabezadoPorDefecto Is Nothing Then
                Program.CopiarObjeto(Of ConfiguracionReceptores)(mobjEncabezadoPorDefecto, objNvoEncabezado)
            Else
                objNvoEncabezado.IdOficina = 0
                objNvoEncabezado.IdReceptor = 0
                objNvoEncabezado.IdReceptores = 0
                objNvoEncabezado.IDReceptorSafyr = 0
                objNvoEncabezado.Codigo_Mesa = 0
                objNvoEncabezado.Sucursal = 0
    ...
    Public Overrides Async Sub ConfirmarBuscar()
        Try
            If Not IsNothing(cb.Codigo_Mesa) Or Not IsNothing(cb.Sucursal) Or
                Not IsNothing(cb.Nombre) Or Not IsNothing(cb.Codigo) Then 'Validar que ingresó algo en los campos de búsqueda
                Await ConsultarEncabezado(False, String.Empty, cb.Codigo, CInt(cb.Sucursal), CInt(cb.Codigo_Mesa), cb.Nombre)
            End If
        Catch ex As Exception
            IsBusy = False
    ...
    Public Overrides Sub ActualizarRegistro()
        ...
        Try
            ...
            If ValidarRegistro() Then
                ...
                intNroOcurrencias = (From e In ListaEncabezado Where e.IdReceptores = _EncabezadoSeleccionado.IdReceptores Select e).Count
    ...
    Public Overrides Sub BorrarRegistro()
        Try
            If Not IsNothing(_EncabezadoSeleccionado) Then
                ...
                A2Utilidades.Mensajes.mostrarMensajePregunta("Está opción borra el receptor seleccionado. ¿Confirma el borrado de este receptor?", Program.TituloSistema, ValoresUserState.Borrar.ToString(), AddressOf BorrarRegistroConfirmado)
            End If
        Catch ex As Exception
            IsBusy = False
        ...
        Try
            If CType(sender, A2Utilidades.wppMensajePregunta).DialogResult Then
                If Not IsNothing(_EncabezadoSeleccionado) Then
                    ...
                    If mdcProxy.ConfiguracionReceptores.Where(Function(i) i.IdReceptores = EncabezadoSeleccionado.IdReceptores).Count > 0 Then
                        mdcProxy.ConfiguracionReceptores.Remove(mdcProxy.ConfiguracionReceptores.Where(Function(i) i.IdReceptores = EncabezadoSeleccionado.IdReceptores).First)
                    End If
...
#Region "Métodos privados del encabezado - REQUERIDOS"
    ...
    Private Sub PrepararNuevaBusqueda()
        Try
            Dim objCB As New CamposBusquedaConfiguracionReceptores
            objCB.Codigo = String.Empty
            objCB.Codigo_Mesa = 0
            objCB.Sucursal = 0
            objCB.Nombre = Nothing
            cb = objCB
        Catch ex As Exception
            IsBusy = False
    ...
    ''' </summary>
    ''' 
    Private Function ObtenerRegistroAnterior() As ConfiguracionReceptores
        Dim objEncabezado As ConfiguracionReceptores = New ConfiguracionReceptores

        Try
            If Not IsNothing(_EncabezadoSeleccionado) Then
                Program.CopiarObjeto(Of ConfiguracionReceptores)(_EncabezadoSeleccionado, objEncabezado)
                objEncabezado.IdReceptor = _EncabezadoSeleccionado.IdReceptor
            End If
        Catch ex As Exception
...
Public Class CamposBusquedaConfiguracionReceptores
