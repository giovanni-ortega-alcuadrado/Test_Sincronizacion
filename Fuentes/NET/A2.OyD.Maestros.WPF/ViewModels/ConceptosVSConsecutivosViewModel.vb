...
Public Class ConceptosVSConsecutivosViewModel
    ...
    Public Sub New()
        ...
        Try
            If Not Program.IsDesignMode() Then
                ...
                MostrarBuscadorConceptos = Visibility.Collapsed
                MostrarCompañia = Visibility.Collapsed
                logHabilitarGrid = False
                dcProxyUtil.Load(dcProxyUtil.cargarCombosEspecificosQuery("ConcepVsConse", Program.Usuario, Program.HashConexion), AddressOf TerminoCargarCombosEspecificos, "")
            End If
...
#Region "Métodos"
    ...
    Public Sub SeleccionarTodo(blnIsChequed As Boolean)
        Try
            If Not IsNothing(ListaConceptosVSConsecutivos) Then
                For Each it In ListaConceptosVSConsecutivos
    ...
    Private Function ValidarDatos() As Boolean
        ...
        Try
            ...
            '-- Validar Datos
            '-------------------------------------------------------------------------------------------------------------------------
            If Not IsNothing(_ListaConceptosVSConsecutivos) Then
                'Modulos
                ...
                'Concepto
                If strModulos = "T" And lngIDConcepto = 0 Then
                    strMsg = String.Format("{0}{1} + Debe seleccionar un Concepto para continuar.", strMsg, vbCrLf)
                End If
    ...
    Public Overrides Async Sub ActualizarRegistro()
        ...
        Try
            ...
            If ValidarDatos() Then
                ...
                If ListaConceptosVSConsecutivos.Count > 0 Then

                    xmlCompleto = "<ConceptosVsConsecutivos>"

                    If strModulos = "T" Then
                        For Each objeto In (From c In ListaConceptosVSConsecutivos)
                            xmlDetalle = "<Detalle strNombreConsecutivo=""" & objeto.NombreConsecutivo &
                                         """ lngIdConcepto=""" & lngIDConcepto &
                    ...
                    Else
                        ...
                        For Each objeto In (From c In ListaConceptosVSConsecutivos)
                            'For Each Objeto As ConceptosVSConsecutivos In ListaConceptosVSConsecutivos
                            xmlDetalle = "<Detalle strNombreConsecutivo=""" & strNombreConsecutivo &
                                         """ lngIdConcepto=""" & objeto.IDConcepto &
                                         """ logSeleccionado=""" & IIf(CBool(objeto.Seleccionado), 1, 0) &
                                         """ logEvaluar=""" & 1 & """ ></Detalle>"
                            xmlCompleto = xmlCompleto & xmlDetalle
                        Next
                    ...
                    xmlCompleto = xmlCompleto & "</ConceptosVsConsecutivos>"


                    Dim objRet As InvokeOperation(Of String)
                    Dim strMsg As String = String.Empty
                    objRet = Await dcProxy.ConceptosVsConsecutivosActualizar(strModulos, xmlCompleto, Program.Usuario, Program.HashConexion).AsTask()
...
#Region "Resultados Asincrónicos"
    ...
    Private Sub BorrarDatosGrid()
        Try
            ...
            logHabilitarGrid = False
            If Not IsNothing(ListaConceptosVSConsecutivos) Then
                ListaConceptosVSConsecutivos = Nothing
                ListaConceptosVSConsecutivosPaginada = Nothing
            End If
        Catch ex As Exception
    ...
    Public Async Function FiltrarDatos() As Task
        If _strModulos = "T" Then
            If Not IsNothing(_lngIDConcepto) Then
                strUsuarioProceso = strCondicionTipo
                strNombreConsecutivo = String.Empty
                Await ConsultarDetalle(strNombreConsecutivo, lngIDConcepto)
            Else
            ...
            End If
        Else
            If _strCondicionTipo <> String.Empty Then
    ...
    ''' </summary>
    Public Async Function ConsultarDetalle(pstrNombreConsecutivo As String, plngIDConcepto As Integer) As Task(Of Boolean)
        Dim logResultado As Boolean = False
        Dim objRet As LoadOperation(Of ConceptosVSConsecutivos)

        Try
            If Not dcProxy.ConceptosVSConsecutivos Is Nothing Then
                dcProxy.ConceptosVSConsecutivos.Clear()
            End If


            objRet = Await dcProxy.Load(dcProxy.ConceptosVSConsecutivosFiltrarQuery(plngIDConcepto, pstrNombreConsecutivo, Program.Usuario, Program.HashConexion)).AsTask()

            If Not objRet Is Nothing Then
                If objRet.HasError Then
                    If objRet.Error Is Nothing Then
                        A2Utilidades.Mensajes.mostrarMensaje("Se generó un problema al consultar el detalle de los conceptos. ", Program.TituloSistema, A2Utilidades.wppMensajes.TiposMensaje.Errores)
                    Else
                        A2Utilidades.Mensajes.mostrarErrorAplicacion(Program.Usuario, "Se generó un problema al consultar el detalle de los conceptos vs consectivos.", Me.ToString(), "ConsultarDetalle", Program.TituloSistema, Program.Maquina, objRet.Error)
                    End If
                Else
                    ListaConceptosVSConsecutivos = objRet.Entities.ToList
                End If
            End If
            logResultado = True
        Catch ex As Exception
            A2Utilidades.Mensajes.mostrarErrorAplicacion(Program.Usuario, "Se presentó un problema en la recepción del detalle de los conceptos vs consectivos.", Me.ToString(), "ConsultarDetalle", Application.Current.ToString(), Program.Maquina, ex)
        End Try
...
#Region "Propiedades"
    ...
    ''' </summary>
    Private _MostrarBuscadorConceptos As Visibility = Visibility.Collapsed
    Public Property MostrarBuscadorConceptos() As Visibility
        Get
            Return _MostrarBuscadorConceptos
        End Get
        Set(value As Visibility)
            _MostrarBuscadorConceptos = value
            MyBase.CambioItem("MostrarBuscadorConceptos")
        End Set
    End Property
    ''' <summary>
    ...
    ''' </summary>
    Private _lngIDConcepto As Integer = 0
    Public Property lngIDConcepto() As Integer
        ...
        Set(ByVal value As Integer)
            ...
            'ActualizarGridPermisos()
            strCondicionTipo = _strNombreConcepto
            BorrarDatosGrid()
            MyBase.CambioItem("lngIDConcepto")
        End Set
    End Property

    Private _strNombreConcepto As String = String.Empty
    Public Property strNombreConcepto() As String
        Get
            Return _strNombreConcepto
        End Get
        Set(ByVal value As String)
            _strNombreConcepto = value
            'FiltrarDatos()

            BorrarDatosGrid()
            MyBase.CambioItem("strNombreConcepto")
        End Set
    End Property
    ''' <summary>
    ''' Esta propiedad Contiene la lista de los datos consecutivos y usuarios''' 
    ''' </summary>
    Private _ListaConceptosVSConsecutivos As List(Of ConceptosVSConsecutivos)
    Public Property ListaConceptosVSConsecutivos As List(Of ConceptosVSConsecutivos)
        Get
            Return _ListaConceptosVSConsecutivos
        End Get
        Set(value As List(Of ConceptosVSConsecutivos))
            _ListaConceptosVSConsecutivos = value
            MyBase.CambioItem("ListaConceptosVSConsecutivos")
            MyBase.CambioItem("ListaConceptosVSConsecutivosPaginada")
        End Set
    End Property
    ''' <summary>
    ''' Esta propiedad contiene los items detallados del grid''' 
    ''' </summary>
    Public Property ListaConceptosVSConsecutivosPaginada() As PagedCollectionView
        Get
            If Not IsNothing(_ListaConceptosVSConsecutivos) Then
                Dim view = New PagedCollectionView(_ListaConceptosVSConsecutivos)
                Return view
            Else
                Return Nothing
            End If
    ...
    ''' </summary>
    Private WithEvents _ListaConceptosVSConsecutivosSeleccionado As ConceptosVSConsecutivos
    Public Property ListaConceptosVSConsecutivosSeleccionado() As ConceptosVSConsecutivos
        Get
            Return _ListaConceptosVSConsecutivosSeleccionado
        End Get
        Set(ByVal value As ConceptosVSConsecutivos)
            _ListaConceptosVSConsecutivosSeleccionado = value
            MyBase.CambioItem("ListaConceptosVSConsecutivosSeleccionado")
        End Set
    End Property
