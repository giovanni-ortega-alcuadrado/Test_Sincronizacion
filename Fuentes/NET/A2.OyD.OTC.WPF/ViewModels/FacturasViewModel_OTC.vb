...
Public Class FacturasViewModel_OTC
    Inherits A2ControlMenu.A2ViewModel
    Public Property cb As New CamposBusquedaFactura
    Public Property TotalFactu As New TotalFactura
    Private FacturaPorDefecto As Factura_OTC
    Private FacturaAnterior As Factura_OTC
...
#Region "Resultados Asincrónicos"
    ...
    Private Sub TerminoTraerFacturasPorDefecto_Completed(ByVal lo As LoadOperation(Of Factura_OTC))
    ...
    Private Sub TerminoTraerFacturas(ByVal lo As LoadOperation(Of Factura_OTC))
        If Not lo.HasError Then
            ListaFacturas = dcProxy.Factura_OTCs
            If dcProxy.Factura_OTCs.Count > 0 Then
    ...
    Private Sub TerminoTraerLiquidaciones(ByVal lo As LoadOperation(Of Liquidaciones_OT))
        If Not lo.HasError Then
            ListaLiquidaciones = dcProxy.Liquidaciones_OTs
...
#Region "Propiedades"
    ...
    Private _ListaFacturas As EntitySet(Of Factura_OTC)
    Public Property ListaFacturas() As EntitySet(Of Factura_OTC)
        Get
            Return _ListaFacturas
        End Get
        Set(ByVal value As EntitySet(Of Factura_OTC))
    ...
    Private _FacturaSelected As Factura_OTC
    Public Property FacturaSelected() As Factura_OTC
        Get
            Return _FacturaSelected
        End Get
        Set(ByVal value As Factura_OTC)
            ...
            If Not value Is Nothing Then
                ...
                dcProxy.Load(dcProxy.Liquidaciones_FacturaOTC_ConsultarQuery(FacturaSelected.Numero, FacturaSelected.Prefijo, Program.Usuario, Program.HashConexion), AddressOf TerminoTraerLiquidaciones, Nothing)
            Else
                ListaLiquidaciones = dcProxy.Liquidaciones_OTs
            End If
    ...
    Public Overrides Sub ConfirmarBuscar()
        Try
            If cb.Numero <> 0 Or cb.Comitente <> String.Empty Or (Not IsNothing(cb.Fecha)) Or cb.NombreComitente <> String.Empty Then 'Validar que ingresó algo en los campos de búsqueda
                ...
                dcProxy.Load(dcProxy.FacturasOTC_ConsultarQuery(cb.Numero, cb.Comitente, cb.Fecha, cb.NombreComitente, Program.Usuario, Program.HashConexion), AddressOf TerminoTraerFacturas, "Busqueda")
...
#Region "Tablas hijas"
    ...
    '******************************************************** Liquidaciones 
    Private _ListaLiquidaciones As EntitySet(Of Liquidaciones_OT)
    Public Property ListaLiquidaciones() As EntitySet(Of Liquidaciones_OT)
        Get
    ...
    Private _LiquidacioneSelected As Liquidaciones_OT
    Public Property LiquidacioneSelected() As Liquidaciones_OT
        Get
    ...
    Public Overrides Sub NuevoRegistroDetalle()
        Select Case NombreColeccionDetalle
            Case "cmLiquidacione"
                Dim NewLiquidacione As New Liquidaciones_OT
