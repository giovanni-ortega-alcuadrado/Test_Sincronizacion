...
'''
'''</history>
Public Class InstrumentosDetalleViewModel
...
#Region "Variables - REQUERIDO"
    ...
    '---------------------------------------------------------------------------------------------------------------------------------------------------
    Private mobjEncabezadoPorDefecto As tblProductosVencimientos
    ...
    '---------------------------------------------------------------------------------------------------------------------------------------------------
    Private mobjEncabezadoAnterior As tblProductosVencimientos
    Private mobjEncabezadoGuardadoAnterior As tblProductosVencimientos
    Public viewListaPrincipal As InstrumentosDetalleView
...
#Region "Propiedades del Encabezado - REQUERIDO"
    ...
    ''' </summary>
    Private _ListaEncabezado As List(Of tblProductosVencimientos)
    Public Property ListaEncabezado() As List(Of tblProductosVencimientos)
        Get
    ...
    ''' </summary>
    Private _EncabezadoSeleccionado As tblProductosVencimientos
    Public Property EncabezadoSeleccionado() As tblProductosVencimientos
        Get
            Return _EncabezadoSeleccionado
        End Get
        Set(ByVal value As tblProductosVencimientos)
    ...
    ''' </summary>
    Private WithEvents _EncabezadoEdicionSeleccionado As tblProductosVencimientos
    Public Property EncabezadoEdicionSeleccionado() As tblProductosVencimientos
        Get
...
#Region "Métodos públicos del encabezado - REQUERIDO"
    ...
    Public Sub CrearNuevoDetalle(Optional ByVal plogCopiarValorRegistroAnterior As Boolean = False)
        Try
            Dim objNvoEncabezado As New tblProductosVencimientos
            ...
            objNvoEncabezado.intIdProducto = _IDRegistroEncabezado
    ...
    Public Async Function ActualizarRegistroDetalle() As Task(Of Boolean)
        ...
        Try
            ...
            Dim objRespuesta As InvokeResult(Of List(Of CPX_tblValidacionesGenerales)) = Await mdcProxy.ProductosVencimientos_ActualizarAsync(EncabezadoEdicionSeleccionado.intIdProductoVencimiento,
                                                                                                                                               _EncabezadoEdicionSeleccionado.intIdProducto,
                                                                                                                                               _EncabezadoEdicionSeleccionado.strVencimiento,
                                                                                                                                               _EncabezadoEdicionSeleccionado.strNemotecnico,
                                                                                                                                               _EncabezadoEdicionSeleccionado.dtmFechaVencimiento,
                                                                                                                                               _EncabezadoEdicionSeleccionado.logActivo,
                                                                                                                                               _EncabezadoEdicionSeleccionado.intUltimoDiaNegPV,
                                                                                                                                               _EncabezadoEdicionSeleccionado.dtmFechaUltDiaNeg,
                                                                                                                                               False,
        ...
        Try
            ...
            If CType(sender, A2Utilidades.wppMensajePregunta).DialogResult Then
                If Not IsNothing(_EncabezadoSeleccionado) Then
                    ...
                    Dim objRespuesta As InvokeResult(Of List(Of CPX_tblValidacionesGenerales)) = Await mdcProxy.ProductosVencimientos_EliminarAsync(_EncabezadoSeleccionado.intIdProductoVencimiento, Program.Usuario, Program.HashConexion)
    ...
    Public Sub AbrirDetalle()
        Try
            If logVentanaDetalleActiva = False Then
                Dim objViewDetalle As New InstrumentosDetalleFormView(Me)
...
#Region "Métodos privados del encabezado - REQUERIDOS"
    ...
    ''' <history>
    ''' </history>
    Private Sub ObtenerRegistroAnterior(ByVal pobjRegistro As tblProductosVencimientos, ByRef pobjRegistroSalvar As tblProductosVencimientos)
        Dim objEncabezado As tblProductosVencimientos = New tblProductosVencimientos
        ...
        Try
            If Not IsNothing(pobjRegistro) Then
                ...
                objEncabezado.strVencimiento = pobjRegistro.strVencimiento
                ...
                objEncabezado.intUltimoDiaNegPV = pobjRegistro.intUltimoDiaNegPV
                objEncabezado.dtmFechaUltDiaNeg = pobjRegistro.dtmFechaUltDiaNeg
...
#Region "Métodos sincrónicos del encabezado - REQUERIDO"
    ...
    Private Async Sub consultarEncabezadoPorDefecto()
        Try
            Dim objRespuesta As InvokeResult(Of tblProductosVencimientos)

            objRespuesta = Await mdcProxy.ProductosVencimientos_DefectoAsync(Program.Usuario, Program.HashConexion)
    ...
    Private Async Function ConsultarEncabezado(ByVal pstrOpcion As String,
                                               ByVal pintIdProducto As Integer,
                                               Optional ByVal pstrFiltro As String = "",
                                               Optional ByVal pintIdProductoVencimiento As Integer = 0,
                                               Optional ByVal pstrVencimiento As String = "",
                                               Optional ByVal pstrNemotecnico As String = "",
                                               Optional ByVal plogActivo As Boolean = 1
                                               ) As Task(Of Boolean)
        ...
        Try
            IsBusy = True
            Dim objRespuesta As InvokeResult(Of List(Of tblProductosVencimientos)) = Nothing
            ...
            If pstrOpcion = "FILTRAR" Then
                ...
                objRespuesta = Await mdcProxy.ProductosVencimientos_FiltrarAsync(pintIdProducto, pstrFiltro, Program.Usuario, Program.HashConexion)

            Else
                pstrVencimiento = System.Web.HttpUtility.UrlEncode(Program.validarValorString(pstrVencimiento, String.Empty))
                pstrNemotecnico = System.Web.HttpUtility.UrlEncode(Program.validarValorString(pstrNemotecnico, String.Empty))

                objRespuesta = Await mdcProxy.ProductosVencimientos_ConsultarAsync(pintIdProducto, pstrVencimiento, pstrNemotecnico, plogActivo, Program.Usuario, Program.HashConexion)
            End If
            ...
            If Not IsNothing(objRespuesta) Then
                If Not IsNothing(objRespuesta.Value) Then
                    ...
                    If pintIdProductoVencimiento > 0 Then
                        If ListaEncabezado.Where(Function(i) i.intIdProductoVencimiento = pintIdProductoVencimiento).Count > 0 Then
                            EncabezadoSeleccionado = ListaEncabezado.Where(Function(i) i.intIdProductoVencimiento = pintIdProductoVencimiento).First
                        End If
                    Else
    ...
    Public Async Sub ConsultarEncabezadoEdicion()
        Try
            ...
            If logCambiarEncabezadoSeleccionado Then
                If Not IsNothing(_EncabezadoSeleccionado) Then
                    If _EncabezadoSeleccionado.intIdProductoVencimiento > 0 Then
                        Await ConsultarEncabezadoEdicion(_EncabezadoSeleccionado.intIdProductoVencimiento)
                    End If
                End If
            End If
        Catch ex As Exception
    ...
    Private Async Function ConsultarEncabezadoEdicion(ByVal pintID As Integer) As Task(Of Boolean)
        ...
        Try
            IsBusy = True
            Dim objRespuesta As InvokeResult(Of tblProductosVencimientos) = Nothing

            ErrorForma = String.Empty

            objRespuesta = Await mdcProxy.ProductosVencimientos_IDAsync(pintID, Program.Usuario, Program.HashConexion)
