...
'''
'''</history>
Public Class MensajesPoliticaViewModel
...
#Region "Variables - REQUERIDO"
    ...
    '---------------------------------------------------------------------------------------------------------------------------------------------------
    Private mobjEncabezadoPorDefecto As tblMensajesCumPolitica
    ...
    '---------------------------------------------------------------------------------------------------------------------------------------------------
    Private mobjEncabezadoAnterior As tblMensajesCumPolitica
...
#Region "Propiedades del Encabezado - REQUERIDO"
    ...
    ''' </summary>
    Private _ListaEncabezado As List(Of CPX_tblMensajesCumPolitica)
    Public Property ListaEncabezado() As List(Of CPX_tblMensajesCumPolitica)
        Get
            Return _ListaEncabezado
        End Get
        Set(ByVal value As List(Of CPX_tblMensajesCumPolitica))
    ...
    ''' </summary>
    Private _EncabezadoSeleccionado As CPX_tblMensajesCumPolitica
    Public Property EncabezadoSeleccionado() As CPX_tblMensajesCumPolitica
        Get
            Return _EncabezadoSeleccionado
        End Get
        Set(ByVal value As CPX_tblMensajesCumPolitica)
    ...
    ''' Elemento de la lista de Registros que se encuentra seleccionado
    ''' </summary>
    Private WithEvents _EncabezadoEdicionSeleccionado As tblMensajesCumPolitica
    Public Property EncabezadoEdicionSeleccionado() As tblMensajesCumPolitica
        Get
            Return _EncabezadoEdicionSeleccionado
        End Get
        Set(ByVal value As tblMensajesCumPolitica)
            _EncabezadoEdicionSeleccionado = value
            If Not IsNothing(_EncabezadoEdicionSeleccionado) Then
                If _EncabezadoEdicionSeleccionado.intidMensajeCumPolitica > 0 Then
    ...
    ''' </summary>
    Private _cb As CamposBusquedaMensajesPolitica
    Public Property cb() As CamposBusquedaMensajesPolitica
        Get
            Return _cb
        End Get
        Set(ByVal value As CamposBusquedaMensajesPolitica)
            _cb = value
...
#Region "Métodos públicos del encabezado - REQUERIDO"
    ...
    Public Overrides Sub NuevoRegistro()
        Try
            Dim objNvoEncabezado As New tblMensajesCumPolitica
    ...
    Public Overrides Async Sub ConfirmarBuscar()
        Try
            If cb.intEstadoCumplimiento > 0 Or Not String.IsNullOrEmpty(cb.strMensaje) Then
                Await ConsultarEncabezado("BUSQUEDA", String.Empty, 0, cb.intEstadoCumplimiento, cb.strMensaje)
    ...
    Public Overrides Async Sub ActualizarRegistro()
        ...
        Try
            ...
            Dim objRespuesta As InvokeResult(Of List(Of CPX_tblValidacionesGenerales)) = Await mdcProxy.MensajesCumPolitica_ActualizarAsync(_EncabezadoEdicionSeleccionado.intidMensajeCumPolitica,
                                                                                                                                            _EncabezadoEdicionSeleccionado.intEstadoCumplimiento,
                                                                                                                                            _EncabezadoEdicionSeleccionado.strMensaje,
                                                                                                                                             False,
                ...
                If Not IsNothing(objRespuesta.Value) Then
                    ...
                    If (objListaResultado.Where(Function(i) i.logInconsitencia = True).Count) = 0 Then
                        ...
                        ElseIf strTipoFiltroBusqueda = "BUSQUEDA" And Not IsNothing(cb) Then
                            Await ConsultarEncabezado("BUSQUEDA", String.Empty, Nothing, cb.intEstadoCumplimiento, cb.strMensaje)
                        Else
        ...
        Try
            ...
            If CType(sender, A2Utilidades.wppMensajePregunta).DialogResult Then
                If Not IsNothing(_EncabezadoSeleccionado) Then
                    ...
                    Dim objRespuesta As InvokeResult(Of List(Of CPX_tblValidacionesGenerales)) = Await mdcProxy.MensajesCumPolitica_EliminarAsync(_EncabezadoSeleccionado.intID, Program.Usuario, Program.HashConexion)
...
#Region "Métodos privados del encabezado - REQUERIDOS"
    ...
    Private Sub PrepararNuevaBusqueda()
        Try
            Dim objCB As New CamposBusquedaMensajesPolitica
            objCB.intEstadoCumplimiento = -1
            objCB.strMensaje = ""
            cb = objCB
        Catch ex As Exception
            IsBusy = False
    ...
    ''' <history>
    ''' </history>
    Private Sub ObtenerRegistroAnterior(ByVal pobjRegistro As tblMensajesCumPolitica, ByRef pobjRegistroSalvar As tblMensajesCumPolitica)
        Dim objEncabezado As tblMensajesCumPolitica = New tblMensajesCumPolitica

        Try
            If Not IsNothing(pobjRegistro) Then

                objEncabezado.intidMensajeCumPolitica = pobjRegistro.intidMensajeCumPolitica
                objEncabezado.intEstadoCumplimiento = pobjRegistro.intEstadoCumplimiento
                objEncabezado.strMensaje = pobjRegistro.strMensaje
...
#Region "Métodos sincrónicos del encabezado - REQUERIDO"
    ...
    Private Async Sub consultarEncabezadoPorDefecto()
        Try
            Dim objRespuesta As InvokeResult(Of tblMensajesCumPolitica)

            objRespuesta = Await mdcProxy.MensajesCumPolitica_DefectoAsync(Program.Usuario, Program.HashConexion)
    ...
    Private Async Function ConsultarEncabezado(ByVal pstrOpcion As String,
                                               ...
                                               Optional ByVal pintEstadoCumplimiento As Integer = -1,
                                               Optional ByVal pstrMensaje As String = "") As Task(Of Boolean)

        Dim logResultado As Boolean = False

        Try
            IsBusy = True
            Dim objRespuesta As InvokeResult(Of List(Of CPX_tblMensajesCumPolitica)) = Nothing
            ...
            If pstrOpcion = "FILTRAR" Then
                ...
                objRespuesta = Await mdcProxy.MensajesCumPolitica_FiltrarAsync(pstrFiltro, Program.Usuario, Program.HashConexion)

            Else
                pstrMensaje = System.Web.HttpUtility.UrlEncode(Program.validarValorString(pstrMensaje, String.Empty))

                objRespuesta = Await mdcProxy.MensajesCumPolitica_ConsultarAsync(pstrMensaje, pintEstadoCumplimiento, Program.Usuario, Program.HashConexion)
            End If
    ...
    Private Async Function ConsultarEncabezadoEdicion(ByVal pintID As Integer) As Task(Of Boolean)
        ...
        Try
            IsBusy = True
            Dim objRespuesta As InvokeResult(Of tblMensajesCumPolitica) = Nothing

            ErrorForma = String.Empty

            objRespuesta = Await mdcProxy.MensajesCumPolitica_IDAsync(pintID, Program.Usuario, Program.HashConexion)
...
''' 
''' </history>
Public Class CamposBusquedaMensajesPolitica
    Implements INotifyPropertyChanged

    Private _intEstadoCumplimiento As Integer
    Public Property intEstadoCumplimiento() As Integer
        Get
            Return _intEstadoCumplimiento
        End Get
        Set(ByVal value As Integer)
            _intEstadoCumplimiento = value
            RaiseEvent PropertyChanged(Me, New PropertyChangedEventArgs("intEstadoCumplimiento"))
        End Set
    End Property
    ...
    Public Property strMensaje() As String
        ...
        Set(ByVal value As String)
            _strMensaje = value
            RaiseEvent PropertyChanged(Me, New PropertyChangedEventArgs("strMensaje"))
        End Set
    End Property
