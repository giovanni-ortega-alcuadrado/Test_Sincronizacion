...
Partial Public Class FinalidadOperacionView
...
#Region "Variables"
    ...
    Private mobjVM As FinalidadOperacionViewModel
...
#Region "Inicializacion"
    ...
    Public Sub New()
        InitializeComponent()
        mobjVM = New FinalidadOperacionViewModel
    ...
    Private Async Sub inicializar()
        If Not Me.DataContext Is Nothing Then
            mobjVM = CType(Me.DataContext, FinalidadOperacionViewModel)
...
#End Region
    ...
    Private Sub NavegarAForma(sender As Object, e As RoutedEventArgs)
        If Not IsNothing(mobjVM.EncabezadoSeleccionado) Then
            If mobjVM.EncabezadoSeleccionado.intIdFinalidadOperacion.ToString <> CType(sender, Button).Tag Then
                mobjVM.EncabezadoSeleccionado = mobjVM.ListaEncabezado.Where(Function(i) i.intIdFinalidadOperacion.ToString = CType(sender, Button).Tag).First
            End If
        Else
            mobjVM.EncabezadoSeleccionado = mobjVM.ListaEncabezado.Where(Function(i) i.intIdFinalidadOperacion.ToString = CType(sender, Button).Tag).First
        End If
    ...
    Private Sub PermiteComplemento_Checked(sender As Object, e As RoutedEventArgs)
        Try

            mobjVM.Habilitar_Inhabilitar_UbicacionPosicionCubrir()

        Catch ex As Exception
            A2Utilidades.Mensajes.mostrarErrorAplicacion(Program.Usuario, mobjVM.DiccionarioMensajesPantalla("GENERICO_ERRORREGISTRO"), Me.Name, "PermiteComplemento_Checked", Program.TituloSistema, Program.Maquina, ex, Program.RutaServicioLog)
        End Try
    End Sub
End Class
