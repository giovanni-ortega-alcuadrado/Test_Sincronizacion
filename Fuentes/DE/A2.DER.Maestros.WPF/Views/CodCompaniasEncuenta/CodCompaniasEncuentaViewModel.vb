...
'''
'''</history>
Public Class CodCompaniasEncuentaViewModel
...
#Region "Variables - REQUERIDO"
    ...
    '---------------------------------------------------------------------------------------------------------------------------------------------------
    Private mobjEncabezadoPorDefecto As tblCodComEnc
    ...
    '---------------------------------------------------------------------------------------------------------------------------------------------------
    Private mobjEncabezadoAnterior As tblCodComEnc
...
#Region "Propiedades del Encabezado - REQUERIDO"
    ...
    ''' </summary>
    Private _ListaEncabezado As List(Of CPX_tblCodComEnc)
    Public Property ListaEncabezado() As List(Of CPX_tblCodComEnc)
        Get
            Return _ListaEncabezado
        End Get
        Set(ByVal value As List(Of CPX_tblCodComEnc))
    ...
    ''' </summary>
    Private _EncabezadoSeleccionado As CPX_tblCodComEnc
    Public Property EncabezadoSeleccionado() As CPX_tblCodComEnc
        Get
            Return _EncabezadoSeleccionado
        End Get
        Set(ByVal value As CPX_tblCodComEnc)
    ...
    ''' Elemento de la lista de Registros que se encuentra seleccionado
    ''' </summary>
    Private WithEvents _EncabezadoEdicionSeleccionado As tblCodComEnc
    Public Property EncabezadoEdicionSeleccionado() As tblCodComEnc
        Get
            Return _EncabezadoEdicionSeleccionado
        End Get
        Set(ByVal value As tblCodComEnc)
            _EncabezadoEdicionSeleccionado = value
            If Not IsNothing(_EncabezadoEdicionSeleccionado) Then
                If _EncabezadoEdicionSeleccionado.IntIdCodComEnc > 0 Then
    ...
    ''' </summary>
    Private _cb As CamposBusquedaCodComEnc
    Public Property cb() As CamposBusquedaCodComEnc
        Get
            Return _cb
        End Get
        Set(ByVal value As CamposBusquedaCodComEnc)
            _cb = value
...
#Region "Métodos públicos del encabezado - REQUERIDO"
    ...
    Public Overrides Sub NuevoRegistro()
        Try
            Dim objNvoEncabezado As New tblCodComEnc
    ...
    Public Overrides Async Sub ConfirmarBuscar()
        Try
            If cb.intIdPlanoContable > 0 Or Not String.IsNullOrEmpty(cb.strNroDocumento) Or cb.intIdCuentaMultiproducto > 0 Or Not String.IsNullOrEmpty(cb.strCodComEnc) Then
                Await ConsultarEncabezado("BUSQUEDA", String.Empty, 0, cb.intIdPlanoContable, cb.strNroDocumento, cb.intIdCuentaMultiproducto, cb.strCodComEnc)
    ...
    Public Overrides Async Sub ActualizarRegistro()
        ...
        Try
            ...
            Dim objRespuesta As InvokeResult(Of List(Of CPX_tblValidacionesGenerales)) = Await mdcProxy.CodComEnc_ActualizarAsync(_EncabezadoEdicionSeleccionado.IntIdCodComEnc,
                                                                                                                                  _EncabezadoEdicionSeleccionado.IntIdPlanoContable,
                                                                                                                                  _EncabezadoEdicionSeleccionado.IntIdCuentaMultiproducto,
                                                                                                                                  _EncabezadoEdicionSeleccionado.strCodComEnc,
                                                                                                                                  False,
                                                                                                                                  Program.Usuario,
                ...
                If Not IsNothing(objRespuesta.Value) Then
                    ...
                    If (objListaResultado.Where(Function(i) i.logInconsitencia = True).Count) = 0 Then
                        ...
                        ElseIf strTipoFiltroBusqueda = "BUSQUEDA" And Not IsNothing(cb) Then
                            Await ConsultarEncabezado("BUSQUEDA", String.Empty, Nothing, cb.intIdPlanoContable, cb.strNroDocumento, cb.intIdCuentaMultiproducto, cb.strCodComEnc)
                        Else
        ...
        Try
            ...
            If CType(sender, A2Utilidades.wppMensajePregunta).DialogResult Then
                If Not IsNothing(_EncabezadoSeleccionado) Then
                    ...
                    Dim objRespuesta As InvokeResult(Of List(Of CPX_tblValidacionesGenerales)) = Await mdcProxy.CodComEnc_EliminarAsync(_EncabezadoSeleccionado.intID, Program.Usuario, Program.HashConexion)
...
#Region "Métodos privados del encabezado - REQUERIDOS"
    ...
    Private Sub PrepararNuevaBusqueda()
        Try
            Dim objCB As New CamposBusquedaCodComEnc
            objCB.intIdPlanoContable = -1
            objCB.strNroDocumento = ""
            objCB.intIdCuentaMultiproducto = -1
            objCB.strCodComEnc = ""
            cb = objCB
        Catch ex As Exception
            IsBusy = False
    ...
    ''' <history>
    ''' </history>
    Private Sub ObtenerRegistroAnterior(ByVal pobjRegistro As tblCodComEnc, ByRef pobjRegistroSalvar As tblCodComEnc)
        Dim objEncabezado As tblCodComEnc = New tblCodComEnc

        Try
            If Not IsNothing(pobjRegistro) Then

                objEncabezado.IntIdCodComEnc = pobjRegistro.IntIdCodComEnc
                objEncabezado.IntIdPlanoContable = pobjRegistro.IntIdPlanoContable
                objEncabezado.IntIdCuentaMultiproducto = pobjRegistro.IntIdCuentaMultiproducto
                objEncabezado.strCodComEnc = pobjRegistro.strCodComEnc
...
#Region "Métodos sincrónicos del encabezado - REQUERIDO"
    ...
    Private Async Sub consultarEncabezadoPorDefecto()
        Try
            Dim objRespuesta As InvokeResult(Of tblCodComEnc)

            objRespuesta = Await mdcProxy.CodComEnc_DefectoAsync(Program.Usuario, Program.HashConexion)
    ...
    Private Async Function ConsultarEncabezado(ByVal pstrOpcion As String,
                                               ...
                                               Optional ByVal pintIdPlanoContable As Integer = -1,
                                               Optional ByVal pstrNroDocumento As String = "",
                                               Optional ByVal pintIdCuentaMultiproducto As Integer = -1,
                                               Optional ByVal pstrCodComEnc As String = "") As Task(Of Boolean)

        Dim logResultado As Boolean = False

        Try
            IsBusy = True
            Dim objRespuesta As InvokeResult(Of List(Of CPX_tblCodComEnc)) = Nothing
            ...
            If pstrOpcion = "FILTRAR" Then
                ...
                objRespuesta = Await mdcProxy.CodComEnc_FiltrarAsync(pstrFiltro, Program.Usuario, Program.HashConexion)

            Else
                pstrNroDocumento = System.Web.HttpUtility.UrlEncode(Program.validarValorString(pstrNroDocumento, String.Empty))
                pstrCodComEnc = System.Web.HttpUtility.UrlEncode(Program.validarValorString(pstrCodComEnc, String.Empty))

                objRespuesta = Await mdcProxy.CodComEnc_ConsultarAsync(pintIdPlanoContable, pstrNroDocumento, pintIdCuentaMultiproducto, pstrCodComEnc, Program.Usuario, Program.HashConexion)
            End If
    ...
    Private Async Function ConsultarEncabezadoEdicion(ByVal pintID As Integer) As Task(Of Boolean)
        ...
        Try
            IsBusy = True
            Dim objRespuesta As InvokeResult(Of tblCodComEnc) = Nothing

            ErrorForma = String.Empty

            objRespuesta = Await mdcProxy.CodComEnc_IDAsync(pintID, Program.Usuario, Program.HashConexion)
...
''' 
''' </history>
Public Class CamposBusquedaCodComEnc
    Implements INotifyPropertyChanged

    Private _intIdPlanoContable As Integer
    Public Property intIdPlanoContable() As Integer
        Get
            Return _intIdPlanoContable
        End Get
        Set(ByVal value As Integer)
            _intIdPlanoContable = value
            RaiseEvent PropertyChanged(Me, New PropertyChangedEventArgs("intIdPlanoContable"))
        End Set
    End Property
    ...
    Private _intIdCuentaMultiproducto As Integer
    Public Property intIdCuentaMultiproducto() As Integer
        Get
            Return _intIdCuentaMultiproducto
        End Get
        Set(ByVal value As Integer)
            _intIdCuentaMultiproducto = value
            RaiseEvent PropertyChanged(Me, New PropertyChangedEventArgs("intIdCuentaMultiproducto"))
        End Set
    End Property

    Private _strCodComEnc As String
    Public Property strCodComEnc() As String
        Get
            Return _strCodComEnc
        End Get
        Set(ByVal value As String)
            _strCodComEnc = value
            RaiseEvent PropertyChanged(Me, New PropertyChangedEventArgs("strCodComEnc"))
        End Set
    End Property
