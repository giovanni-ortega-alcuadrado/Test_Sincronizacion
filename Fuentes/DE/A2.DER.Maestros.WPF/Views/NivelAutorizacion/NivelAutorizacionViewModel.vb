...
'''
'''</history>
Public Class NivelAutorizacionViewModel
...
#Region "Variables - REQUERIDO"
    ...
    '---------------------------------------------------------------------------------------------------------------------------------------------------
    Private mobjEncabezadoPorDefecto As tblNivelAutorizacion
    ...
    '---------------------------------------------------------------------------------------------------------------------------------------------------
    Private mobjEncabezadoAnterior As tblNivelAutorizacion
...
#Region "Propiedades del Encabezado - REQUERIDO"
    ...
    ''' </summary>
    Private _ListaEncabezado As List(Of CPX_tblNivelAutorizacion)
    Public Property ListaEncabezado() As List(Of CPX_tblNivelAutorizacion)
        Get
            Return _ListaEncabezado
        End Get
        Set(ByVal value As List(Of CPX_tblNivelAutorizacion))
    ...
    ''' </summary>
    Private _EncabezadoSeleccionado As CPX_tblNivelAutorizacion
    Public Property EncabezadoSeleccionado() As CPX_tblNivelAutorizacion
        Get
            Return _EncabezadoSeleccionado
        End Get
        Set(ByVal value As CPX_tblNivelAutorizacion)
    ...
    ''' Elemento de la lista de Registros que se encuentra seleccionado
    ''' </summary>
    Private WithEvents _EncabezadoEdicionSeleccionado As tblNivelAutorizacion
    Public Property EncabezadoEdicionSeleccionado() As tblNivelAutorizacion
        Get
            Return _EncabezadoEdicionSeleccionado
        End Get
        Set(ByVal value As tblNivelAutorizacion)
            _EncabezadoEdicionSeleccionado = value
            If Not IsNothing(_EncabezadoEdicionSeleccionado) Then
                If _EncabezadoEdicionSeleccionado.intidNivelAutorizacion > 0 Then
    ...
    ''' </summary>
    Private _cb As CamposBusquedaNivelAutorizacion
    Public Property cb() As CamposBusquedaNivelAutorizacion
        Get
            Return _cb
        End Get
        Set(ByVal value As CamposBusquedaNivelAutorizacion)
            _cb = value
...
#Region "Métodos públicos del encabezado - REQUERIDO"
    ...
    Public Overrides Sub NuevoRegistro()
        Try
            Dim objNvoEncabezado As New tblNivelAutorizacion
    ...
    Public Overrides Async Sub ConfirmarBuscar()
        Try
            If cb.intIdSucursal > 0 Then
                Await ConsultarEncabezado("BUSQUEDA", String.Empty, 0, cb.intIdSucursal)
    ...
    Public Overrides Async Sub ActualizarRegistro()
        ...
        Try
            ...
            Dim objRespuesta As InvokeResult(Of List(Of CPX_tblValidacionesGenerales)) = Await mdcProxy.NivelAutorizacion_ActualizarAsync(_EncabezadoEdicionSeleccionado.intidNivelAutorizacion,
                                                                                                                                            _EncabezadoEdicionSeleccionado.intidSucursal,
                                                                                                                                            _EncabezadoEdicionSeleccionado.bitAplicaValorInversion,
                                                                                                                                          _EncabezadoEdicionSeleccionado.numRangoInicial,
                                                                                                                                          _EncabezadoEdicionSeleccionado.numRangoFinal,
                                                                                                                                          _EncabezadoEdicionSeleccionado.intNumNivelAutorizacion,
                                                                                                                                             False,
                ...
                If Not IsNothing(objRespuesta.Value) Then
                    ...
                    If (objListaResultado.Where(Function(i) i.logInconsitencia = True).Count) = 0 Then
                        ...
                        ElseIf strTipoFiltroBusqueda = "BUSQUEDA" And Not IsNothing(cb) Then
                            Await ConsultarEncabezado("BUSQUEDA", String.Empty, Nothing, cb.intIdSucursal)
                        Else
        ...
        Try
            ...
            If CType(sender, A2Utilidades.wppMensajePregunta).DialogResult Then
                If Not IsNothing(_EncabezadoSeleccionado) Then
                    ...
                    Dim objRespuesta As InvokeResult(Of List(Of CPX_tblValidacionesGenerales)) = Await mdcProxy.NivelAutorizacion_EliminarAsync(_EncabezadoSeleccionado.intID, Program.Usuario, Program.HashConexion)
...
#Region "Métodos privados del encabezado - REQUERIDOS"
    ...
    Private Sub PrepararNuevaBusqueda()
        Try
            Dim objCB As New CamposBusquedaNivelAutorizacion
            objCB.intIdSucursal = -1
            cb = objCB
        Catch ex As Exception
            IsBusy = False
    ...
    ''' <history>
    ''' </history>
    Private Sub ObtenerRegistroAnterior(ByVal pobjRegistro As tblNivelAutorizacion, ByRef pobjRegistroSalvar As tblNivelAutorizacion)
        Dim objEncabezado As tblNivelAutorizacion = New tblNivelAutorizacion

        Try
            If Not IsNothing(pobjRegistro) Then

                objEncabezado.intidNivelAutorizacion = pobjRegistro.intidNivelAutorizacion
                objEncabezado.intidSucursal = pobjRegistro.intidSucursal
                objEncabezado.bitAplicaValorInversion = pobjRegistro.bitAplicaValorInversion
                objEncabezado.numRangoInicial = pobjRegistro.numRangoInicial
                objEncabezado.numRangoFinal = pobjRegistro.numRangoFinal
                objEncabezado.intNumNivelAutorizacion = pobjRegistro.intNumNivelAutorizacion
...
#Region "Métodos sincrónicos del encabezado - REQUERIDO"
    ...
    Private Async Sub consultarEncabezadoPorDefecto()
        Try
            Dim objRespuesta As InvokeResult(Of tblNivelAutorizacion)

            objRespuesta = Await mdcProxy.NivelAutorizacion_DefectoAsync(Program.Usuario, Program.HashConexion)
    ...
    Private Async Function ConsultarEncabezado(ByVal pstrOpcion As String,
                                               ...
                                               Optional ByVal pintIdSucursal As Integer = -1) As Task(Of Boolean)

        Dim logResultado As Boolean = False

        Try
            IsBusy = True
            Dim objRespuesta As InvokeResult(Of List(Of CPX_tblNivelAutorizacion)) = Nothing
            ...
            If pstrOpcion = "FILTRAR" Then
                ...
                objRespuesta = Await mdcProxy.NivelAutorizacion_FiltrarAsync(pstrFiltro, Program.Usuario, Program.HashConexion)

            Else
                'pstrMensaje = System.Web.HttpUtility.UrlEncode(Program.validarValorString(pstrMensaje, String.Empty))

                objRespuesta = Await mdcProxy.NivelAutorizacion_ConsultarAsync(pintIdSucursal, Program.Usuario, Program.HashConexion)
            End If
    ...
    Private Async Function ConsultarEncabezadoEdicion(ByVal pintID As Integer) As Task(Of Boolean)
        ...
        Try
            IsBusy = True
            Dim objRespuesta As InvokeResult(Of tblNivelAutorizacion) = Nothing

            ErrorForma = String.Empty

            objRespuesta = Await mdcProxy.NivelAutorizacion_IDAsync(pintID, Program.Usuario, Program.HashConexion)
...
''' 
''' </history>
Public Class CamposBusquedaNivelAutorizacion
    Implements INotifyPropertyChanged

    Private _intIdSucursal As Integer
    Public Property intIdSucursal() As Integer
        Get
            Return _intIdSucursal
        End Get
        Set(ByVal value As Integer)
            _intIdSucursal = value
            RaiseEvent PropertyChanged(Me, New PropertyChangedEventArgs("intIdSucursal"))
        End Set
    End Property
