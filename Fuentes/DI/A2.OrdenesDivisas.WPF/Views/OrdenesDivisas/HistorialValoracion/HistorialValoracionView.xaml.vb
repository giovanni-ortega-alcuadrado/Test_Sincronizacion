...
Public Class HistorialValoracionView
#Region "Variables"

    Private mobjVM As HistorialValoracionViewModel
...
#Region "Inicializacion"
    Public Sub New()
        ...
        mobjVM = New HistorialValoracionViewModel
    ...
    Private Async Sub inicializar()
        If Not Me.DataContext Is Nothing Then
            mobjVM = CType(Me.DataContext, HistorialValoracionViewModel)
...
#Region "Propiedades"
    ...
    Public Shared ReadOnly RegistroEncabezadoProperty As DependencyProperty =
                        DependencyProperty.Register("RegistroEncabezado", GetType(tblOrdenes), GetType(HistorialValoracionView), New FrameworkPropertyMetadata() With {.BindsTwoWayByDefault = True, .PropertyChangedCallback = (AddressOf CambioRegistroEncabezado)})
    ...
    Public Shared ReadOnly ListaDetalleProperty As DependencyProperty =
                        DependencyProperty.Register("ListaDetalle", GetType(List(Of CPX_OrdenesValoracionHistoricoConsultar)), GetType(HistorialValoracionView), New FrameworkPropertyMetadata() With {.BindsTwoWayByDefault = True, .PropertyChangedCallback = (AddressOf CambioListaDetalle)})
    Public Property ListaDetalle() As List(Of CPX_OrdenesValoracionHistoricoConsultar)
        Get
            Return CType(GetValue(ListaDetalleProperty), List(Of CPX_OrdenesValoracionHistoricoConsultar))
        End Get
        Set(value As List(Of CPX_OrdenesValoracionHistoricoConsultar))
    ...
    Private Shared Sub CambioListaDetalle(doj As DependencyObject, dp As DependencyPropertyChangedEventArgs)
        Dim myObject = DirectCast(doj, HistorialValoracionView)
